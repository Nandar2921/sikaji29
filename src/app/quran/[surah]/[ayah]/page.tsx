...
// Nama-nama surah untuk metadata
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `QS. ${surahNum}:${ayahNum} - ${surahName} | SiKAJI`,
    description: `Baca surat ${surahName} ayat ${ayahNum} lengkap dengan teks Arab, terjemahan Indonesia, dan tafsir.`,
    openGraph: {
      title: `QS. ${surahNum}:${ayahNum} - ${surahName}`,
      description: `Baca surat ${surahName} ayat ${ayahNum} lengkap dengan terjemahan dan tafsir.`,
    },
  };
}

export default async function VerseDetailPage({ params }: Props) {
  const { surah, ayah } = await params;
  return <VerseDetailClient surah={surah} ayah={ayah} />;
}